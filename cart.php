...
<?php
  if(isset($_POST['submit_D'])){
    $pro_id=$_POST['pro_id'];
    mysqli_query($con,"delete from panier where product_id ='$pro_id'");
    mysqli_close($con);
...
<head> 
    ...
    <title>Vos Produits || MOQAYADA || Troc</title>
...
<table border="0" cellpadding=10  width=80% >
   <tr> 
    <th colspan="2" style="background-color:orange ;" > <center><H2>PANIER</H2></center> </th>
...
<?PHP 
   ...
   while($row = mysqli_fetch_row($result)){
       ...
       echo "<tr  >
             ...
             <form style='width : 200px ;'  method='POST'><input type='hidden' name='pro_id'  value='$product_id'><button type='submit' class='btn btn-primary'  name='submit_D' >Supprimer du panier</button></form>
              </center><br>
             </td> <td> <center>
             <h2>$owner_name</h2> <br>
              </center>
